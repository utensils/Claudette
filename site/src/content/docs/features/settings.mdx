...
Run a custom shell command when a notification fires. The following environment variables are available:

| Variable | Description |
|----------|-------------|
| `$CLAUDETTE_NOTIFICATION_TITLE` | Notification title |
| `$CLAUDETTE_NOTIFICATION_BODY` | Notification body text |
| `$CLAUDETTE_WORKSPACE_ID` | Workspace ID that triggered the notification |
| `$CLAUDETTE_WORKSPACE_NAME` | Workspace display name |

**Example** — send a Slack message:

```bash
curl -X POST "$SLACK_WEBHOOK_URL" \
  -d "{\"text\": \"$CLAUDETTE_NOTIFICATION_TITLE: $CLAUDETTE_NOTIFICATION_BODY\"}"
```
