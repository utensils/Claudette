...
Native agent wakeups and routines are managed by the scheduler described in [Agent Scheduling](/claudette/features/agent-scheduling/). The current surface is CLI and agent-tool driven; scheduled rows persist in the app database and re-arm when Claudette starts.
...
| Rogue worktrees | Section listing orphan worktree directories found under the configured workspace base dir (`worktree_base_dir`, default `~/.claudette/workspaces`) that no Claudette workspace row claims. Each entry shows the inferred repo name (or slug + "unknown repo" tag when the slug no longer maps to a tracked repo), the on-disk size, and the absolute path. Background scan runs on mount. | — |
| Rescan | Re-runs the rogue-worktree scan against the workspace base. | — |
| Delete (per rogue row) | Opens a confirm box, then permanently removes the orphan directory via `std::fs::remove_dir_all`. Refuses anything not strictly under the workspace base or still claimed by a workspace row. | — |
| Archived workspaces per repository | Per-repository count of archived workspaces with on-disk size readouts: a **total** badge (active + archived) plus an `active X · archived Y` split line. Sizes load asynchronously after mount. | — |
| Per-workspace breakdown chevron | Expands a repo row to list every workspace under it with its individual on-disk size and active/archived status. | — |
...
The size readouts use binary units (KiB / MiB / GiB) and come from the `compute_storage_stats` Tauri command, which walks every workspace's worktree directory concurrently on the blocking pool. The rogue scan uses `scan_rogue_worktrees`, which only looks at the configured `worktree_base_dir/<slug>/<wt_name>/` two-level layout — it does not scan arbitrary filesystem locations.

**Rogue worktrees** typically appear when:
